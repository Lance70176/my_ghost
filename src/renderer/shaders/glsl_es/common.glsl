...
// OpenGL ES requires explicit precision qualifiers.
...
vec4 unlinearize_vec4(vec4 linear_color) {
    bvec3 cutoff = lessThanEqual(linear_color.rgb, vec3(0.0031308));
    vec3 higher = pow(linear_color.rgb, vec3(1.0 / 2.4)) * vec3(1.055) - vec3(0.055);
    vec3 lower = linear_color.rgb * vec3(12.92);

    return vec4(mix(higher, lower, cutoff), linear_color.a);
}
...
vec4 load_color(
    ...
    if (linear_blend) color = linearize_vec4(color);
